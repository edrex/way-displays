...
static int libinput_open_restricted(const char *path, int flags, void *data) {
	...
	if (fd <= 0) {
		log_warn("open '%s' failed %d: '%s'", path, errno, strerror(errno));
		return -errno;
	}
...
static void libinput_close_restricted(int fd, void *data) {
	...
	if (close(fd) != 0) {
		log_warn("close failed %d: '%s'", errno, strerror(errno));
	}
}
...
struct libinput *create_libinput_discovery() {
	...
	if (!udev) {
		log_error("unable to create udev context, abandoning laptop lid detection");
		return NULL;
	}

	libinput = libinput_udev_create_context(&libinput_impl, NULL, udev);
	if (!libinput) {
		log_error("unable to create libinput discovery context, abandoning laptop lid detection");
		return NULL;
	}
	...
	if (libinput_udev_assign_seat(libinput, xdg_seat) != 0) {
		log_error("failed to assign seat to libinput, abandoning laptop lid detection");
		return NULL;
	}
...
char *discover_lid_device(struct libinput *libinput) {
	...
	if (libinput_dispatch(libinput) != 0) {
		log_error("failed to dispatch libinput, abandoning laptop lid detection");
		return NULL;
	}
...
bool update_lid(struct Displ *displ) {
	...
	if (libinput_dispatch(displ->lid->libinput_monitor) < 0) {
		log_error("unable to dispatch libinput %d: '%s', abandoning laptop lid detection", errno, strerror(errno));
		destroy_lid(displ);
		return false;
	}
	...
	if (displ->lid->dirty) {
		log_info("Lid %s", displ->lid->closed ? "closed" : "opened");
	}
