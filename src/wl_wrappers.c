...
int _wl_display_prepare_read(struct wl_display *display, char *file, int line) {
	...
	if ((ret = wl_display_prepare_read(display)) == -1) {
		if (errno != EAGAIN) {
			log_error("wl_display_prepare_read failed %d: '%s' at %s:%d, exiting", errno, strerror(errno), file, line);
			exit(EXIT_FAILURE);
		}
	}
...
int _wl_display_dispatch_pending(struct wl_display *display, char *file, int line) {
	...
	if ((ret = wl_display_dispatch_pending(display)) == -1) {
		log_error("wl_display_dispatch_pending failed %d: '%s' at %s:%d, exiting", errno, strerror(errno), file, line);
		exit(EXIT_FAILURE);
	}
...
int _wl_display_flush(struct wl_display *display, char *file, int line) {
	...
	if ((ret = wl_display_flush(display)) == -1) {
		log_error("wl_display_flush failed %d: '%s' at %s:%d, exiting", errno, strerror(errno), file, line);
		exit(EXIT_FAILURE);
	}
...
int _wl_display_read_events(struct wl_display *display, char *file, int line) {
	...
	if ((ret = wl_display_read_events(display)) == -1) {
		if (errno == EPIPE) {
			...
			log_error("wl_display_read_events failed %d: '%s' at %s:%d, exiting", errno, strerror(errno), file, line);
			exit(EXIT_FAILURE);
		}
	}
